...
class RAGxplorer(BaseModel):
    ...
    def load_pdf(self,query,verbose: bool = False, umap_params: dict = None):
        """
        ...
        self._documents.embeddings = self._vectordb_reduced['embeddings']
        self._documents.text = get_docs_2(self._vectordb_reduced)
        self._documents.ids = self._vectordb_reduced['ids']
    ...
    def visualize_query(self, query: str, retrieval_method: str="Estándar", top_k:int=5, query_shape_size:int=5, import_projection_data:pd.DataFrame = None) -> go.Figure:
        ...
        if retrieval_method not in ["Estándar", "HyDE", "Multi_qns"]:
        ...
        self._VizData.query_df = pd.DataFrame({"x": [self._query.original_query_projection[0][0]],
                                      ...
                                      "category": "Consulta original",
                                      "size": query_shape_size})

        if retrieval_method == "Estándar":
        ...
        #self._vectordb_reduced = get_chunks_relevants(self._vectordb,
                                                      #self._query.actual_search_queries,
                                                      #50)

        self._query.retrieved_docs = query_chroma(chroma_collection=self._vectordb_reduced,
                                                  query=self._query.actual_search_queries,
                                                  top_k=top_k)

        self._VizData.base_df.loc[self._VizData.base_df['id'].isin(self._query.retrieved_docs), "category"] = "Chunks recuperados"
    ...
    def visualise_query(self, query: str, retrieval_method: str="Estándar", top_k:int=5, query_shape_size:int=5, import_projection_data:pd.DataFrame = None) -> go.Figure:
        """
